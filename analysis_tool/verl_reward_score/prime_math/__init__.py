...
def compute_score_synthetic(synthetic_answer: str, ground_truth: str) -> bool:
    synthetic_answer = str(synthetic_answer)
    ground_truth = str(ground_truth)

    # grade simple algebra questions. if succeeded, return; otherwise, proceed to more complex grading
    if grade_answer(synthetic_answer, ground_truth):
        return True

    try:
        if "\pi" in synthetic_answer or "\pi" in ground_truth:
            equivs = []
            for pi in [math.pi, 3.14]:
                equivs.append(math_equal(synthetic_answer, ground_truth, timeout=True, pi=pi))
            is_correct = any(equivs)
        else:
            is_correct = math_equal(synthetic_answer, ground_truth, timeout=True)
    except:
        is_correct = False
